...
namespace ImGui {
namespace SFML {
void Init(sf::RenderWindow& window, bool loadDefaultFont) { Init(window, window, loadDefaultFont); }
...
void Init(sf::Window& window, const sf::Vector2f& displaySize, bool loadDefaultFont)
{
    ...
    // load mouse cursors
    for (int i = 0; i < ImGuiMouseCursor_COUNT; ++i) { s_mouseCursorLoaded[i] = false; }
...
void ProcessEvent(const sf::Event& event)
{
    if (s_windowHasFocus)
    {
        ...
        switch (event.type)
        {
        case sf::Event::MouseMoved: s_mouseMoved = true; break;
        ...
        case sf::Event::JoystickDisconnected:
            if (s_joystickId == event.joystickConnect.joystickId)
            { // used gamepad was
              // disconnected
                s_joystickId = getConnectedJoystickId();
            }
    ...
    break;
    case sf::Event::GainedFocus: s_windowHasFocus = true; break;
    default: break;
    }
}

void Update(sf::RenderWindow& window, sf::Time dt) { Update(window, window, dt); }
